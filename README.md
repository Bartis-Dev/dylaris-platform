...
| `CORE_SERVICE_NAME` | `core` | No | The name Core answers to on its own Docker network. Only the BYON/route-only deploy snippet uses it: it resolves this name to fill `CORE_GRPC_ADDR`, so the customer gets the service address rather than whichever replica answered. Set it only if the service is not called `core`. |
